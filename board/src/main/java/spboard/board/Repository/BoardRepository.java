...
public interface BoardRepository extends JpaRepository<Board, Long> {
    ...
    Page<Board> findAllByCategoryAndUser_UserRoleNot(BoardCategory category, UserRole userRole, Pageable pageable); // 해당 카테고리에 있는 게시글을 페이지에 맞게 조회, 이 때 ADMIN이 작성한 글(공지) 포함 X
    Page<Board> findAllByCategoryAndTitleContainsAndUser_UserRoleNot(BoardCategory category, String title, UserRole userRole, Pageable pageable); // 해당 카테고리에 있는 공지 글 조회
    Page<Board> findAllByCategoryAndUserNicknameContainsAndUser_UserRoleNot(BoardCategory category, String nickname, UserRole userRole, Pageable pageable); // 검색 기능에 사용
    ...
    List<Board> findAllByUser_LoginIdOrderByCreatedAtDesc(String loginId);
    Long countAllByUser_Id(Long userId);
    Page<Board> findAllByCategory(BoardCategory category, Pageable pageable);

    Page<Board> findAllByCategoryAndTitleContains(
            BoardCategory category, String title, Pageable pageable);
